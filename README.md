...
POS-tagged verses from the Parallel Bible Corpus (PBC; Mayer & Cysouw 2014), with Python code for extracting various metrics and making cross-linguistic comparisons.

Findings are based on metrics computed for each language and aggregated in the file `data/output/All_comparisons_imputed.xlsx` - this contains all measurements for word types in languages of the tagged PBC. The remainder of this Readme describes the process used to obtain these measurements.

To run the basic stats via this repository, install the requirements (recommended in a virtual environment). These requirements do not include all the libraries used for developing the corpus, dependencies for which can be installed manually following scripts in the `processing` folder:
...
- The `data` folder contains multiple subfolders, including:  
  - `output`: a folder to store statistical test results and plots created by scripts in this repository.
  - `tagged`: a folder containing the 1599 POS-tagged portions of Bibles from the PBC.
...
This repository contains data and code to reproduce the results found in the paper "Word length predicts word order". There are two collections of Python scripts within this repository.

### Scripts
The first collection of scripts makes use of the tagged PBC dataset to derive word order patterns for unclassified languages and aggregate observations of noun/argument and verb/predicate lengths for all languages. The resulting spreadsheet file is then used as the basis for statistical tests as reported in the paper. To run these scripts and reproduce the analyses from pre-populated spreadsheets, run the `analyze_taggedPBC.py` script. Alternatively, the spreadsheets can be downloaded separately for processing via a statistical tool of your choice.
...
The second major set of dependencies involve the libraries and databases used to verify the quality of the tags, as well as the alignment of the "N1 ratio" with word order classifications. The tagging libraries include `SpaCy` and `Trankit`, both available via the Python package manager, and the hand-tagged database is the Universal Dependencies Treebank (UDT). Both the tagging libraries require additional downloads and the UDT (version 2.14) must be downloaded separately and pre-processed in order to be used for verification (a script is included in the `processing` subfolder to help with this). The data from the sources used for identifying word order (WALS, AUTOTYP, Grambank) have been converted into separate XLSX spreadsheets in this repository for comparison of their coding with the N1 ratio. These are then collated and used to train the classifiers for imputing word order for the remaining (unclassified) languages in the tagged PBC. The verification procedure can be followed by running the `verify_tagged_PBC.py` script and requires fewer datasets/dependencies. Portions of this code (such as the word order verification) should run without much difficulty.
...
Previous investigations into word order in languages of the world have worked with a relatively small number of languages (the largest sample to date is by [Hahn & Xu 2022](#1) who investigated 80 languages). This makes it difficult to assess general claims about the properties of human language. In order to address this, we can use the large Parallel Bible Corpus (PBC; [Mayer & Cysouw 2014](#2)), which has recently been updated to contain portions of the Bible in over 2000 languages. Of these, 1599 modern languages have at least 3000 verses of the New Testament translated into the language. Unfortunately, these translations have not been tagged for part of speech.
...
1. Reduce the number of verses in the corpus to a) those which share at least 4 lemmas in 2 English translations, then further reduce to verses where b) a verb occurs that is also present in at least three other verses, resulting in 1885 verses for training our word alignment models.
...
By “word order” most researchers refer to the relative order of referential arguments (nouns) versus predicates (verbs), such that it is possible to talk about “verb-initial” vs “verb-final” or “verb-medial” languages, as well as grammatical constructs like Subject and Object in relation to the position of the Verb. Here, we are not strictly talking about the "noun" and "verb" word class, since Subjects and Objects are often pronouns or demonstratives rather than bare nouns. Predicates, similarly, are not formed solely from verbs in all languages.
...
2. In the resulting set, verses/sentences with a noun occurring first are considered to be “noun first”, while those with a verb occurring first are considered “verb first”.
...
#### 2.3 Assessing the validity of the N1 ratio (`verify_tagged_PBC.py`) <a name="assessing-N1-ratio"></a>
...
#### 2.4 Classifying word order for unknown languages (`verify_tagged_PBC.py`) <a name="classifying-word-order"></a>
